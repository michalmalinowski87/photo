...
/**
 * Internal table component that uses hooks properly
 */
function GalleryTableInner<T>({ config }: { config: TableConfig<T> }) {
  ...
  const sortedData = useMemo(() => {
    ...
    return [...data].sort((a, b) => {
      const aValue = column.render
        ? String(column.render(a))
        : String((a as Record<string, unknown>)[localSort.key] ?? "");
      const bValue = column.render
        ? String(column.render(b))
        : String((b as Record<string, unknown>)[localSort.key] ?? "");

      if (aValue < bValue) {
        return localSort.order === "asc" ? -1 : 1;
      }
      if (aValue > bValue) {
        return localSort.order === "asc" ? 1 : -1;
      }
                ...
                {column.sortable ? (
                  ...
                  >
                    ...
                    {localSort?.key === column.key && (
                      <span className="text-gray-400">{localSort.order === "asc" ? "↑" : "↓"}</span>
                    )}
                  </button>
                ) : (
                  column.header
 ...
 */
export function withGalleryTable<P extends object, T = unknown>(
  WrappedComponent: ComponentType<P & WithGalleryTableProps<T>>
) {
  return function GalleryTableComponent(props: P) {
    const renderTable = useCallback((config: TableConfig<T>) => {
      return <GalleryTableInner config={config} />;
    }, []);
