		...
		useEffect(() => {
			...
			getIdToken()
				.then((cognitoToken) => {
					...
					try {
						const payload = JSON.parse(atob(cognitoToken.split('.')[1])) as { sub?: string; 'cognito:username'?: string };
				...
				.catch(() => {
					...
					if (stored) {
						try {
							const payload = JSON.parse(atob(stored.split('.')[1])) as { sub?: string; 'cognito:username'?: string };
							setToken(stored);
							setOwnerId(payload.sub || payload['cognito:username'] || '');
							setCheckingAuth(false);
						} catch (e) {
