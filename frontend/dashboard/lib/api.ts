		...
		if (response.status === 401 && retryOn401 && typeof window !== 'undefined') {
			...
			if (authHeader) {
				try {
					...
					const retryIsJson = retryContentType ? retryContentType.includes('application/json') : false;
					...
					if (!retryResponse.ok) {
						const error = new Error(
							retryBody?.error || retryBody?.message || `HTTP ${retryResponse.status}: ${retryResponse.statusText}`
						) as ApiError;
