...
export function isFinalUrl(url: string, img: ImageFallbackUrls): boolean {
  ...
  if ((img.finalUrl && urlsMatch(url, img.finalUrl)) ||
      (img.url && urlsMatch(url, img.url))) {
    return true;
  }
...
export function getNextFallbackUrl(
  ...
  const tryCloudFront = (size: 'thumb' | 'preview' | 'bigthumb', url: string | null | undefined): string | null => {
  ...
  const tryS3 = (size: 'thumb' | 'preview' | 'bigthumb', url: string | null | undefined): string | null => {
    if (hasAttemptedSize(size)) {
      return null;
    }
    if (!url || urlsMatch(failedUrl, url)) {
