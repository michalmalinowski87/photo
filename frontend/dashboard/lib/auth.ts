import { CognitoUserPool, CognitoUser, AuthenticationDetails, CognitoUserAttribute, CognitoRefreshToken, ISignUpResult, CognitoUserSession } from 'amazon-cognito-identity-js';
...
function reconstructCognitoSession(): boolean {
	...
	if (!clientId || !idToken) return false;
					...
					if (refreshToken) {
						...
						refreshIdToken()
							...
							.catch((refreshErr) => {
								...
								reject(refreshErr || new Error('Token expired and refresh failed'));
							});
						return;
					}
				}
			...
			if (allowRefresh && typeof window !== 'undefined') {
				...
				if (refreshToken) {
					...
					refreshIdToken()
						.then(resolve)
						.catch((refreshErr) => {
							reject(refreshErr || new Error('No user logged in and refresh failed'));
						});
					return;
				}
			}
				...
				if (allowRefresh && typeof window !== 'undefined') {
					...
					if (refreshToken) {
						...
						refreshIdToken()
							.then(resolve)
							.catch((refreshErr) => {
								reject(refreshErr || err || new Error('Invalid session'));
							});
						return;
					}
				}
	...
	return new Promise((resolve, reject) => {
		...
		user.getSession((err: Error | null, session: CognitoUserSession | null) => {
			if (err || !session || !session.isValid()) {
				...
				user.refreshSession(cognitoRefreshToken, (refreshErr: Error | null, newSession: CognitoUserSession | null) => {
					...
					if (clientId) {
						try {
							...
							const extractedUsername = idTokenPayload['cognito:username'] || idTokenPayload.email || idTokenPayload.sub;
							
							if (extractedUsername) {
								sessionStorage.setItem(`CognitoIdentityServiceProvider.${clientId}.LastAuthUser`, extractedUsername);
								sessionStorage.setItem(`CognitoIdentityServiceProvider.${clientId}.${extractedUsername}.idToken`, newIdToken);
								sessionStorage.setItem(`CognitoIdentityServiceProvider.${clientId}.${extractedUsername}.accessToken`, newAccessToken);
								if (newRefreshToken) {
									sessionStorage.setItem(`CognitoIdentityServiceProvider.${clientId}.${extractedUsername}.refreshToken`, newRefreshToken);
								}
							}
						} catch (e) {
							// Failed to update sessionStorage, but tokens are in localStorage
							console.warn('Failed to update sessionStorage after token refresh:', e);
						}
					}
...
export function signUp(email: string, password: string): Promise<ISignUpResult['user']> {
	return new Promise((resolve, reject) => {
		...
		userPool.signUp(email, password, attributeList, [], (err: Error | null, result: ISignUpResult | null) => {
			if (err) {
				reject(err);
				return;
			}
