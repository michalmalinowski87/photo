 ...
 * 
 * @param {string} plnString - PLN value as string (e.g., "5.50")
 * @returns {number} - Value in cents (e.g., 550)
 */
export function plnToCents(plnString) {
 ...
 * 
 * @param {number} cents - Value in cents (e.g., 550)
 * @returns {string} - PLN value as string (e.g., "5.50") or empty string for 0
 */
export function centsToPlnString(cents) {
  if (cents === 0) return '';
...
export function isValidCurrency(value) {
  if (!value || value === '') return true; // Empty is valid (will be treated as 0)
