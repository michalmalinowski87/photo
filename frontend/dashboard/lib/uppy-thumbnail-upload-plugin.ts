 ...
 * 1. Thumbnail (120x120, square): Uses Uppy's ThumbnailGenerator (via thumbnail:generated event)
 ...
 * - Thumbnail (120x120, square, quality 80, adaptive if >20KB): ~6-10KB
 ...
 *   - Smaller size (120px) prevents upscaling small images, faster generation, optimized for speed
 *
 ...
 *
 * Why this approach:
 * - Uppy's ThumbnailGenerator: Best for square 120px thumbnails (uses Canvas API internally, optimized)
 *   - 120px prevents upscaling small images, maintaining crisp quality
 *   - Faster generation than 150px, optimized for speed while maintaining good quality
...
export class ThumbnailUploadPlugin extends BasePlugin<any, any, any> {
   ...
   * This gives us the 120px thumbnail that we can use directly
  ...
  private async handleUploadSuccess(file: any) {
    ...
    try {
      ...
      const [preview, bigThumb, thumbnailBlob] = await Promise.all([
        ...
        // Thumbnail (120x120) - from Uppy's ThumbnailGenerator
  ...
  private async getThumbnailBlob(file: any, fileSizeMB?: number): Promise<Blob | null> {
    try {
      ...
      // Uppy's ThumbnailGenerator stores the 120px thumbnail in file.preview (data URL)
      ...
      if (blob.type !== "image/webp") {
        blob = await this.convertToWebP(blob, 120, 0.8);
      }
  ...
  private async optimizeThumbnailSize(blob: Blob, fileSizeMB: number): Promise<Blob> {
    try {
      ...
      const compressedFile = await imageCompression(file, {
        ...
        maxWidthOrHeight: 120, // Maintain 120px dimension
        useWebWorker: false,
        fileType: "image/webp",
