 ...
 * 1. Thumbnail (150x150, square): Uses Uppy's ThumbnailGenerator (via thumbnail:generated event)
 ...
 * - Thumbnail (150x150, square, quality 80, adaptive if >20KB): ~8-12KB
 ...
 *   - Smaller size (150px) prevents upscaling small images, faster generation, still excellent quality
 *
 ...
 *
 * Why this approach:
 * - Uppy's ThumbnailGenerator: Best for square 150px thumbnails (uses Canvas API internally, optimized)
 *   - 150px prevents upscaling small images, maintaining crisp quality
 *   - Faster generation than 200px, still provides excellent quality for larger images when downscaled
...
export class ThumbnailUploadPlugin extends BasePlugin<any, any, any> {
   ...
   * This gives us the 150px thumbnail that we can use directly
  ...
  private async handleUploadSuccess(file: any) {
    ...
    try {
      ...
      const [preview, bigThumb, thumbnailBlob] = await Promise.all([
        ...
        // Thumbnail (150x150) - from Uppy's ThumbnailGenerator
  ...
  private async getThumbnailBlob(file: any, fileSizeMB?: number): Promise<Blob | null> {
    try {
      ...
      // Uppy's ThumbnailGenerator stores the 150px thumbnail in file.preview (data URL)
      ...
      if (blob.type !== "image/webp") {
        blob = await this.convertToWebP(blob, 150, 0.8);
      }
  ...
  private async optimizeThumbnailSize(blob: Blob, fileSizeMB: number): Promise<Blob> {
    try {
      ...
      const compressedFile = await imageCompression(file, {
        ...
        maxWidthOrHeight: 150, // Maintain 150px dimension
        useWebWorker: false,
        fileType: "image/webp",
