 ...
 * 1. Thumbnail (300x300, square): Generated using browser-image-compression for high quality
 ...
 * - Thumbnail (300x300, square, high quality 0.90-0.95): ~20-80KB
 ...
 *   - Square crop maintains aspect ratio while ensuring consistent sizing
 *   - High quality prevents pixelation even at 300x300 display size
 *
...
export class ThumbnailUploadPlugin extends BasePlugin<any, any, any> {
  ...
  private async handleUploadSuccess(file: any) {
    ...
    try {
      ...
      const [preview, bigThumb, thumbnailBlob] = await Promise.all([
        ...
        // Thumbnail (300x300) - custom generation for high quality dashboard thumbs
   ...
   *
   * Uses high quality settings (0.90-0.95) to prevent pixelation at 300x300 display size.
   * Prioritizes quality over file size - even at high quality, 300x300 thumbnails remain small.
   * Generates square thumbnails by cropping to center while maintaining aspect ratio.
   *
  ...
  private async generateThumbnail(file: any, fileSizeMB?: number): Promise<Blob | null> {
    ...
    try {
      ...
      // Quality range: 0.90-0.95 (high quality for 300x300 thumbnails)
      ...
      // First, resize to fit within 300px while maintaining aspect ratio
      // Remove maxSizeMB constraint to prioritize quality - 300x300 thumbnails will naturally be small
      const compressedFile = await imageCompression(fileWithMeta.data, {
        maxSizeMB: 1, // Increased limit to prevent quality reduction - 300x300 will still be small
        maxWidthOrHeight: 300, // Fit inside 300px, maintain aspect ratio
        useWebWorker: false,
        fileType: "image/webp",
        initialQuality: quality,
      });

      // Convert to blob and create square thumbnail by cropping
      const blob = await compressedFile.arrayBuffer();
      const imageBitmap = await createImageBitmap(new Blob([blob], { type: "image/webp" }));
      ...
      if (!ctx) {
        // Fallback: return the compressed file as-is if canvas is not available
        return new Blob([blob], { type: "image/webp" });
      }

      // Calculate crop dimensions to center the image and create square thumbnail
      ...
      if (sourceAspect > 1) {
        // Source is wider - crop width (take center square from width)
        cropSize = sourceHeight; // Use full height
        sourceX = (sourceWidth - cropSize) / 2; // Center horizontally
        sourceY = 0;
      } else {
        // Source is taller or square - crop height (take center square from height)
        cropSize = sourceWidth; // Use full width
        sourceX = 0;
        sourceY = (sourceHeight - cropSize) / 2; // Center vertically
      }
