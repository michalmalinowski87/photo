 ...
 * 1. Thumbnail (300px, maintain ratio, quality 0.5): Generated using browser-image-compression
 * 2. BigThumb (700px, maintain ratio, quality 0.9): Uses browser-image-compression library
...
export class ThumbnailUploadPlugin extends BasePlugin<any, any, any> {
  ...
  private async handleUploadSuccess(file: any) {
    ...
    try {
      ...
      const [preview, bigThumb, thumbnailBlob] = await Promise.all([
        ...
        // BigThumb (700px) - for masonry/responsive grid layouts (quality 0.9)
        this.generateBigThumb(fileWithMeta),
        // Thumbnail (300px) - for dashboard thumbnails (quality 0.5)
  ...
  /**
   * Generate thumbnail (300px, maintain ratio) from image using browser-image-compression
   * For dashboard thumbnails and CMS grid views
   *
   * Uses fixed quality 0.5 for consistent compression. No optimization target to prevent
   * double compression or over-compression. Does not upscale images smaller than 300px.
  ...
  private async generateThumbnail(file: any): Promise<Blob | null> {
    ...
    try {
      // Resize to fit within 300px while maintaining aspect ratio
      // Quality 0.5 for consistent compression, no maxSizeMB to prevent double compression
      // browser-image-compression does not upscale by default
      const compressedFile = await imageCompression(fileWithMeta.data, {
        maxWidthOrHeight: 300, // Fit inside 300px, maintain aspect ratio, no upscaling
        useWebWorker: false,
        fileType: "image/webp",
        initialQuality: 0.5, // Fixed quality 0.5
      });
  ...
  /**
   * Generate BigThumb (700px) from image using browser-image-compression
   ...
   * double compression or over-compression. Does not upscale images smaller than 700px.
   * Maintains aspect ratio (not square cropped).
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private async generateBigThumb(file: any): Promise<Blob | null> {
    ...
    try {
      // Resize to fit within 700px while maintaining aspect ratio
      // Quality 0.9 for consistent compression, no maxSizeMB to prevent double compression
      // browser-image-compression does not upscale by default
      const compressedFile = await imageCompression(fileWithMeta.data, {
        maxWidthOrHeight: 700, // Fit inside 700px, maintain aspect ratio, no upscaling
        useWebWorker: false,
        fileType: "image/webp",
