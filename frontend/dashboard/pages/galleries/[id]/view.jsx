...
function OwnerGalleryView({ token, ownerId, galleryId, mode }) {
	...
	async function loadGallery() {
		...
		try {
			...
			try {
				...
				setGalleryName(gallery.galleryName || galleryId);
				setGalleryInfo(gallery);
			} catch (e) {
				// Gallery info not available, that's OK
				setGalleryName(galleryId);
			}
		} catch (error) {
		...
		<div style={{ padding: 24, maxWidth: '100%', boxSizing: 'border-box' }}>
			...
			{viewMode === 'processed' && (
				<ProcessedPhotosView
					...
					apiFetch={(url, options) => apiFetchWithAuth(url, options, token)}
				/>
			)}
