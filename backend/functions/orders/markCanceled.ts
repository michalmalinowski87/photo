...
export const handler = lambdaLogger(async (event: any) => {
	...
	// Don't allow canceling already canceled or delivered orders
	if (order.deliveryStatus === 'CANCELLED') {
		return {
			statusCode: 400,
			headers: { 'content-type': 'application/json' },
			body: JSON.stringify({ error: 'Order is already canceled' })
		};
	}
	if (order.deliveryStatus === 'DELIVERED') {
		return {
			statusCode: 400,
			headers: { 'content-type': 'application/json' },
			body: JSON.stringify({ error: 'Cannot cancel a delivered order' })
		};
	}
	...
	await ddb.send(new UpdateCommand({
		TableName: ordersTable,
		Key: { galleryId, orderId },
		UpdateExpression: 'SET deliveryStatus = :s, canceledAt = :t',
